...
function AlumnusPage({ AnumnusPageData }) {
  ...
  useEffect(() => {
    setExpandedIndices(new Array(AnumnusPageData.length).fill(false));
  }, [AnumnusPageData]);
      ...
      <section className="partners-data w-100">
        <div className="container-fluid">
          <div className="all-partners w-100 d-flex flex-column" style={{ marginTop: "-40px" }}>
            <h3 className="text-center">Hear from the alumnus</h3>
            <div className="row w-100 justify-content-center">
              {AnumnusPageData.map((item, index) => {
                const maxLength = 150;
                const bio = item.bioData;
                const needsTruncation = bio.length > maxLength;
                const truncatedText = needsTruncation ? bio.slice(0, maxLength) + '...' : bio;
                const isExpanded = expandedIndices[index];

                return (
                  <div key={index} className="col-12 col-md-4 col-lg-4 d-flex justify-content-center">
                    <div className="alumnus-grid-item">
                      <p className="my-2 body1" style={{ lineHeight: "30px" }}>
                        {isExpanded ? bio : truncatedText}
                        {needsTruncation && (
                          <button
                      ...
                      <div className="d-flex justify-content-start ml-3 mt-4">
                        <div>
                          <img src={item.img} className="alumnus-img" alt={item.name} />
                        </div>
                        <div className="ml-3">
